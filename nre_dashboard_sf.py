"""
NRE Sales & Invoice Dashboard — Snowflake-native Streamlit version.

Data is loaded directly via the active Snowpark session (no connector,
no secrets.toml, no parquet cache needed).  Deploy this file to a
Snowflake stage and point a STREAMLIT object at it.
"""
...
# Auto-refresh every 15 minutes via browser meta-refresh
...
# ---------------------------------------------------------------------------
# Data loading — cached for 15 min, uses active Snowpark session
# ---------------------------------------------------------------------------
session = get_active_session()

@st.cache_data(ttl=900, show_spinner=False)
def fetch_data():
    df_opps = session.sql(OPPS_SQL).to_pandas()
    df_inv  = session.sql(INVOICES_SQL).to_pandas()
    df_li   = session.sql(LINE_ITEMS_SQL).to_pandas()
...
with st.spinner("Loading data from Snowflake…"):
    df_opps_raw, df_inv_raw, df_li_raw = fetch_data()
...
st.sidebar.markdown(
    ...
    <b>Source:</b> Live Snowflake<br>
    <b>Data as of:</b> {_now:%Y-%m-%d %H:%M}
    </div>
    """,
    unsafe_allow_html=True,
)
...
# Re-filter opps so NRE Contract Value reacts to year/status changes
...
def sort_controls(key: str):
    ...
    sort_label = c1.selectbox("Sort by", list(SORT_FIELDS.keys()), key=f"sort_field_{key}")
    order      = c2.selectbox("Order", ["Descending", "Ascending"], key=f"sort_order_{key}")
...
with c3:
    ...
    by_grp = (
        ...
        .agg(NRE=("nre_contract_value", "sum"), Count=("opportunity_id", "count"))
    )
...
with c4:
    ...
    trend_metric = st.selectbox(
        "Highlight metric", ["Invoiced", "Paid", "Remaining", "Count"], key="trend_metric"
    )
    fig_trend = go.Figure([
...
# ---------------------------------------------------------------------------
# Row 4 — Billed Year x Status Heatmap
# ---------------------------------------------------------------------------
st.subheader("Billed Year x Status — Invoice Amount Heatmap")
...
heat_metric = st.radio("Heatmap value", ["Amount", "Count"], horizontal=True, key="heat_metric")
if not pivot.empty:
...
fig_li = go.Figure([
    go.Bar(name="Invoiced",  y=li_desc["description"], x=li_desc["Invoiced"],  orientation="h", marker_color="#4C78A8"),
    go.Bar(name="Paid",      y=li_desc["description"], x=li_desc["Paid"],      orientation="h", marker_color="#54A24B"),
    go.Bar(name="Remaining", y=li_desc["description"], x=li_desc["Remaining"], orientation="h", marker_color="#E45756"),
])
